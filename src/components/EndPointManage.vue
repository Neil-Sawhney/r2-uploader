    ...
    <details :open="panelOpen === '1'" id="panel">
        ...
        <form action="javascript:" class="mb-0">
          <div class="pb-2 text-xs opacity-80">
            <span v-show="endPointList.length === 1"
              >You have 1 endpoint, and it is selected.</span
            >
            <span v-show="endPointList.length > 1">
              Choose between different endpoints,
              {{ "you have " + endPointList.length + " endpoints" }}
            </span>
          </div>
          <div v-show="endPointList.length === 0" class="text-sm">
            You need Cloudflare Workers to upload files to Cloudflare R2. Have a look at
            the
          ...
          <div v-for="item in endPointList" class="flex mt-2">
            <input
              ...
              :id="item.endPoint"
              :data-id="item.endPoint"
              :checked="item.endPoint === endPoint"
              @change="updateCurrentEndPoint(item.endPoint)"
              class="w-[2rem] shrink-0"
            /><label :for="item.endPoint" class="text-xs w-full">{{
              item.endPoint
            }}</label>
            ...
            <button
              ...
              style="padding: 0; border: 0"
              @click="deleteThisEndPoint(item.endPoint)"
      ...
      <article>
        <form class="mb-0" action="javascript:" @submit="saveApiInfo">
          <div class="pb-2 text-xs opacity-80">{{ endpointActionText }} &nbsp;</div>
          <div>
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="no need for the https:// prefix"
          ...
          <div class="text-center mt-4">
            <button
              class="inline-block w-auto text-sm mb-0"
...
let editThisEndpoint = function (endpoint) {
  ...
  newCustomDomain.value =
    endPointList.value.find((item) => item.endPoint === endpoint).customDomain || "";
...
const saveApiInfo = function () {
  ...
  if (duplicate) {
    ...
    endPointList.value = endPointList.value.filter(
      (item) => item.endPoint !== newEndpoint.value,
    );
