<template>
  <div>
    <div class="pb-2 text-xs opacity-80">Sync Endpoints</div>
    <div aria-busy="true" v-show="findingUser"></div>
    <div class="text-sm" v-show="!isLogin && !findingUser">
      To sync your endpoints across devices, you can login with your GitHub account. All
      the information will be AES encrypted locally and then stored in our database.
    </div>
    <div class="mt-6" v-show="!isLogin && !findingUser">
      <button class="text-sm inline-block w-auto mb-0" @click="goToLogin">
        Login With
        <img
    ...
    <div v-show="isLogin">
      <div class="text-sm mb-4 mt-4">Welcome, {{ userInfo.login }}!</div>
      <div>
        <div>
          <div class="text-xs mb-4" v-show="!hasEncryptionPassword">
            Since now you've logged in, you can sync your endpoints across devices. But
            first, you need to provide a password to encrypt and decrypt your data.
            Remember, this password will be stored locally and will not be sent to our
            server. Meaning
            <b>if you forget it, you'll lose all previously saved data</b>.
          </div>
          <form action="javascript:" @submit="saveEncryptionPassword">
            <label for="en_password" class="text-sm"
              >Encryption/Decryption Password</label
            >
            <div class="flex">
              <input
                class="mb-0"
                ...
                v-if="!showPassword"
                @focus="encryptionPasswordInputFocus = true"
                @blur="encryptionPasswordInputFocus = false"
              />
              <input
                class="mb-0"
                type="text"
              ...
              />
              <button
                class="shrink-0 text-xs outline inline-block w-auto ml-2"
                type="submit"
              >
            ...
            <div>
              <input type="checkbox" v-model="showPassword" id="show_password" />
              <label for="show_password" class="text-xs">Show password</label>
            </div>
          </form>
        </div>
        <div class="text-xs">
          <button
            class="inline-block w-auto text-xs mb-2"
            :disabled="!hasEncryptionPassword || syncingMyData"
            @click="syncMyData"
            :aria-busy="syncingMyData"
          >
            🔼 Push
          </button>
          <button
            class="inline-block w-auto text-xs ml-2 mb-2"
            :disabled="!hasEncryptionPassword || pullingMyData"
            @click="pullMyData"
            :aria-busy="pullingMyData"
          >
            🔽 Pull
          </button>
          <br />
          <button class="inline-block w-auto text-xs outline" @click="logout">
            Log Out
          </button>
          <button
            class="ml-2 inline-block w-auto text-xs outline border-red-500 text-red-500"
            @click="deleteMyData"
            :disabled="deletingMyData"
            :aria-busy="deletingMyData"
          >
            Delete all synced endpoints
          </button>
        </div>
        <div class="text-xs opacity-50 mt-2">status: {{ sync_status }}</div>
      </div>
    </div>
  </div>
</template>
...
let pullMyData = async function () {
  ...
  try {
    ...
    try {
      ...
      animateText(
        sync_status,
        "pulled, but the data seems to be corrupted, check your password please",
      );
      return false;
    }
